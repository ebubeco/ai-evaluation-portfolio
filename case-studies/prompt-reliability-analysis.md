...
The report contained 7 findings. The summary addresses 5, partially. Two findings are either absent or rolled into vague summary language ("additional findings highlight..."). The model resolved the impossible constraint of 7 substantial findings in under 150 words by silently dropping content rather than flagging the tension.
...
This failure mode is difficult to detect because the output passes surface-level checks. It is fluent, correctly formatted, and appears complete.

It is not.

The model has satisfied the most easily measurable constraint (word count and formatting) while failing the most important one; completeness of content. In the absence of explicit structure, the model resolves conflicting instructions by optimizing for what can be verified locally within the response.

This behaviour is not an edge case. It is a default pattern under constraint ambiguity. Without clear prioritisation in the prompt, completeness is consistently sacrificed for compliance.
