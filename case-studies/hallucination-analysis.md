...
This is a confidence-without-grounding failure. The model produces a response that is structurally correct and rhetorically strong, but not anchored to verifiable evidence.

The failure is not lack of knowledge, it is failure to distinguish between what is known and what is inferred. When that boundary collapses, the model fills gaps with plausible fabrication.

The UN Women anachronism is the clearest indicator: the model generated an institutional reference without validating whether that institution existed at the claimed date. This is pattern-matching on credibility signals ("UN Women," "ten-year review," "2005") rather than grounded retrieval.

This failure mode is common in responses involving well-known figures and events. Strong signal on the topic creates confidence in the output, while weaker signal on the underlying documentation leads to fabricated specificity.
